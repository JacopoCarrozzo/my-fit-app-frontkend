...
export default function Step6_AgeInput({ onNext, onBack }: StepProps) {
  ...
  return (
    <StepContainer>
      ...
      <StepTitle children={Strings.onboarding.age} />

      <Input
        value={selection}
        placeholder={Strings.onboarding.age2}
        onChangeText={(text) => setSelection(text.replace(/[^0-9]/g, ''))}
        keyboardType="numeric"
      />

      <PrimaryButton text={Strings.onboarding.next} disabled={!selection} onPress={() => onNext({ age: parseInt(selection, 10) })} />
    </StepContainer>
  );
}
